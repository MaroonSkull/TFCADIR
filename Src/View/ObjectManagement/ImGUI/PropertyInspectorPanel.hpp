...
public:
   ...
   * @param selectionManager Reference to selection manager for querying selection
   * @param model Reference to figure model for accessing figure data
   */
  PropertyInspectorPanel(UIFSMAdapter& fsmAdapter, SelectionManager& selectionManager, model::FlatFigures& model);
...
private:
  ...
  struct PropertyDef {
    ...
    std::string type;            ///< Type identifier: "float", "vec2", "vec3", "bool", "int", "string"
    bool isComponent = false;    ///< True if this is a component property (e.g., "center.x")
    int componentIndex = -1;     ///< Component index for vector properties (0=x, 1=y, 2=z)
   ...
   */
  std::vector<PropertyGroup> buildPropertyGroups(std::shared_ptr<model::IFigure> figure);
   ...
   */
  std::vector<PropertyGroup> buildCommonPropertyGroups(const std::vector<std::shared_ptr<model::IFigure>>& figures);

  /**
   * @brief Build properties for a Triangle figure
   */
  void buildTriangleProperties(std::vector<PropertyGroup>& groups, std::shared_ptr<model::IFigure> figure);

  /**
   * @brief Build properties for a Quad figure
   */
  void buildQuadProperties(std::vector<PropertyGroup>& groups, std::shared_ptr<model::IFigure> figure);

  /**
   * @brief Build properties for a Circle figure
   */
  void buildCircleProperties(std::vector<PropertyGroup>& groups, std::shared_ptr<model::IFigure> figure);

  /**
   * @brief Build properties for an Ngon figure
   */
  void buildNgonProperties(std::vector<PropertyGroup>& groups, std::shared_ptr<model::IFigure> figure);

  /**
   * @brief Build properties for a CurveBezier3 figure
   */
  void buildCurveBezier3Properties(std::vector<PropertyGroup>& groups, std::shared_ptr<model::IFigure> figure);

  /**
   * @brief Build properties for a CurveBezier4 figure
   */
  void buildCurveBezier4Properties(std::vector<PropertyGroup>& groups, std::shared_ptr<model::IFigure> figure);
   ...
   */
  void addTransformProperties(std::vector<PropertyDef>& properties, std::shared_ptr<model::IFigure> figure);

  /**
   * @brief Add common appearance properties (visible, color, scribed)
   */
  void addAppearanceProperties(std::vector<PropertyDef>& properties, std::shared_ptr<model::IFigure> figure);

  /**
   * @brief Add common layer property
   */
  void addLayerProperty(std::vector<PropertyDef>& properties, std::shared_ptr<model::IFigure> figure);
   ...
   */
  void addVec3Property(std::vector<PropertyDef>& properties, const std::string& name, const std::string& path, const glm::vec3& value);
   ...
   * @param value Current value
   */
  void addVec2Property(std::vector<PropertyDef>& properties, const std::string& name, const std::string& path, const glm::vec2& value);

  // === Property filtering ===

  /**
   * @brief Check if two property definitions are equivalent (same path and type)
   */
  bool arePropertiesEquivalent(const PropertyDef& a, const PropertyDef& b) const;
   ...
   */
  std::vector<PropertyGroup> filterCommonProperties(const std::vector<PropertyGroup>& groups1, const std::vector<PropertyGroup>& groups2);
