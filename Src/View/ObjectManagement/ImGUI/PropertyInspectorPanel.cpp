...
PropertyInspectorPanel::PropertyInspectorPanel(UIFSMAdapter& fsmAdapter, SelectionManager& selectionManager, model::FlatFigures& model)
    : fsmAdapter_(fsmAdapter), selectionManager_(selectionManager), model_(model),
...
void PropertyInspectorPanel::renderFigureProperties(SelectionState state) {
  ...
  if (state == MultiSelection) {
    ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.0f, 1.0f), "%zu figures selected", selectedIds.size());
    ImGui::Text("Editing common properties of %s", getFigureTypeName(model_.getFigure(primaryId)).c_str());
    ImGui::Separator();
  } else {
    auto primaryFigure = model_.getFigure(primaryId);
    if (primaryFigure) {
      ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.0f, 1.0f), "%s", getFigureTypeName(primaryFigure).c_str());
  ...
  for (const auto& group : propertyCache_.groups) {
    ...
    if (ImGui::CollapsingHeader(group.name.c_str(), ImGuiTreeNodeFlags_DefaultOpen)) {
      for (const auto& prop : group.properties) {
        ...
        if (!filterText_.empty() && prop.name.find(filterText_) == std::string::npos) {
          continue;
        }
...
std::vector<PropertyInspectorPanel::PropertyGroup> PropertyInspectorPanel::buildPropertyGroups(std::shared_ptr<model::IFigure> figure) {
  std::vector<PropertyGroup> groups;

  if (auto triangle = std::dynamic_pointer_cast<model::Figure<model::Triangle>>(figure)) {
  ...
  } else if (auto curve3 = std::dynamic_pointer_cast<model::Figure<model::CurveBezier3>>(figure)) {
    buildCurveBezier3Properties(groups, figure);
  } else if (auto curve4 = std::dynamic_pointer_cast<model::Figure<model::CurveBezier4>>(figure)) {
...
std::vector<PropertyInspectorPanel::PropertyGroup> PropertyInspectorPanel::buildCommonPropertyGroups(
...
void PropertyInspectorPanel::buildTriangleProperties(std::vector<PropertyGroup>& groups, std::shared_ptr<model::IFigure> figure) {
  auto triangle = std::dynamic_pointer_cast<model::Figure<model::Triangle>>(figure);
  if (!triangle) {
    return;
  }
  ...
  addVec3Property(geometryGroup.properties, "First Vertex", "first", pointToVec3(triangle->first));
  addVec3Property(geometryGroup.properties, "Second Vertex", "second", pointToVec3(triangle->second));
  addVec3Property(geometryGroup.properties, "Third Vertex", "third", pointToVec3(triangle->third));
...
void PropertyInspectorPanel::buildQuadProperties(std::vector<PropertyGroup>& groups, std::shared_ptr<model::IFigure> figure) {
  ...
  addVec3Property(geometryGroup.properties, "First Vertex", "first", pointToVec3(quad->first));
  addVec3Property(geometryGroup.properties, "Second Vertex", "second", pointToVec3(quad->second));
  addVec3Property(geometryGroup.properties, "Third Vertex", "third", pointToVec3(quad->third));
  addVec3Property(geometryGroup.properties, "Fourth Vertex", "fourth", pointToVec3(quad->fourth));
...
void PropertyInspectorPanel::buildCircleProperties(std::vector<PropertyGroup>& groups, std::shared_ptr<model::IFigure> figure) {
  ...
  // Radius property
  geometryGroup.properties.push_back({"Radius", "radius", "float", false, -1, circle->radius});

  // Center as vec3 property (v1.3 FIX: include explicit center.z)
  addVec3Property(geometryGroup.properties, "Center", "center", pointToVec3(circle->center));
...
void PropertyInspectorPanel::buildNgonProperties(std::vector<PropertyGroup>& groups, std::shared_ptr<model::IFigure> figure) {
  ...
  // Center property
  addVec3Property(geometryGroup.properties, "Center", "center", pointToVec3(ngon->center));

  // Radius property
  geometryGroup.properties.push_back({"Radius", "radius", "float", false, -1, ngon->radius});

  // Number of sides property (n is float in Ngon struct)
  geometryGroup.properties.push_back({"Sides", "n", "float", false, -1, ngon->n});
...
void PropertyInspectorPanel::buildCurveBezier3Properties(std::vector<PropertyGroup>& groups, std::shared_ptr<model::IFigure> figure) {
  auto curve = std::dynamic_pointer_cast<model::Figure<model::CurveBezier3>>(figure);
  if (!curve) {
    return;
  }
  ...
  addVec3Property(geometryGroup.properties, "Start Point", "start", pointToVec3(curve->start));
  addVec3Property(geometryGroup.properties, "End Point", "end", pointToVec3(curve->end));
  addVec3Property(geometryGroup.properties, "Control Point", "first", pointToVec3(curve->first));
...
void PropertyInspectorPanel::buildCurveBezier4Properties(std::vector<PropertyGroup>& groups, std::shared_ptr<model::IFigure> figure) {
  auto curve = std::dynamic_pointer_cast<model::Figure<model::CurveBezier4>>(figure);
  if (!curve) {
    return;
  }
  ...
  // Control points as vec3 properties (CurveBezier4 has start, end, first, second)
  addVec3Property(geometryGroup.properties, "Start Point", "start", pointToVec3(curve->start));
  addVec3Property(geometryGroup.properties, "End Point", "end", pointToVec3(curve->end));
  addVec3Property(geometryGroup.properties, "Control Point 1", "first", pointToVec3(curve->first));
  addVec3Property(geometryGroup.properties, "Control Point 2", "second", pointToVec3(curve->second));
...
void PropertyInspectorPanel::addTransformProperties(std::vector<PropertyDef>& properties, std::shared_ptr<model::IFigure> figure) {
  // ID (read-only)
  properties.push_back({"ID", "id", "int", false, -1, static_cast<int>(figure->getId())});

  // Name
  properties.push_back({"Name", "name", "string", false, -1, figure->getName()});

  // Position
  addVec3Property(properties, "Position", "position", figure->getPosition());
}

void PropertyInspectorPanel::addAppearanceProperties(std::vector<PropertyDef>& properties, std::shared_ptr<model::IFigure> figure) {
  // Scribed state
  auto figureTyped = std::dynamic_pointer_cast<model::Figure<model::Triangle>>(figure);
  if (figureTyped) {
...
void PropertyInspectorPanel::addLayerProperty(std::vector<PropertyDef>& properties, std::shared_ptr<model::IFigure> figure) {
  // Layer property - not currently available in Figure template
  // Placeholder for future implementation
}

void PropertyInspectorPanel::addVec3Property(std::vector<PropertyDef>& properties, const std::string& name, const std::string& path, const glm::vec3& value) {
...
void PropertyInspectorPanel::addVec2Property(std::vector<PropertyDef>& properties, const std::string& name, const std::string& path, const glm::vec2& value) {
  // Full value property
  PropertyDef fullProp;
  fullProp.name = name;
  fullProp.path = path;
...
bool PropertyInspectorPanel::arePropertiesEquivalent(const PropertyDef& a, const PropertyDef& b) const {
  return a.path == b.path && a.type == b.type;
}

std::vector<PropertyInspectorPanel::PropertyGroup> PropertyInspectorPanel::filterCommonProperties(
    const std::vector<PropertyGroup>& groups1, const std::vector<PropertyGroup>& groups2) {

  std::vector<PropertyGroup> commonGroups;

  for (const auto& group1 : groups1) {
    // Find matching group in groups2
    auto it = std::find_if(groups2.begin(), groups2.end(),
    ...
    for (const auto& prop1 : group1.properties) {
      auto propIt = std::find_if(it->properties.begin(), it->properties.end(),
          [&prop1, this](const PropertyDef& p) { return arePropertiesEquivalent(prop1, p); });
...
bool PropertyInspectorPanel::renderProperty(const PropertyDef& prop, uint32_t figureId) {
  if (prop.type == "float") {
...
bool PropertyInspectorPanel::renderFloatProperty(const PropertyDef& prop, uint32_t figureId) {
...
bool PropertyInspectorPanel::renderVec2Property(const PropertyDef& prop, uint32_t figureId) {
...
bool PropertyInspectorPanel::renderVec3Property(const PropertyDef& prop, uint32_t figureId) {
...
bool PropertyInspectorPanel::renderBoolProperty(const PropertyDef& prop, uint32_t figureId) {
...
bool PropertyInspectorPanel::renderIntProperty(const PropertyDef& prop, uint32_t figureId) {
...
bool PropertyInspectorPanel::renderStringProperty(const PropertyDef& prop, uint32_t figureId) {
...
bool PropertyInspectorPanel::renderColorProperty(const PropertyDef& prop, uint32_t figureId) {
  glm::vec3 value = std::any_cast<glm::vec3>(prop.value);
...
void PropertyInspectorPanel::onPropertyChanged(uint32_t figureId, const std::string& path) {
  propertyCache_.isValid = false;
}

// === Helper functions ===

std::string PropertyInspectorPanel::getFigureTypeName(std::shared_ptr<model::IFigure> figure) {
  ...
  } else if (std::dynamic_pointer_cast<model::Figure<model::CurveBezier3>>(figure)) {
    return model::CurveBezier3::name;
  } else if (std::dynamic_pointer_cast<model::Figure<model::CurveBezier4>>(figure)) {
