...
void GLProgram::checkProgram(GLenum pname, std::string_view msg) const {
  ...
  if (!success) {
    ...
    err << "Program #" << programId_ << " " << msg << ": " << std::endl
        << infoLog;
    throw std::runtime_error{err.str()};
  }
}

GLProgram::GLProgram() { programId_ = glCreateProgram(); }

GLProgram::~GLProgram() { glDeleteProgram(programId_); }
...
void GLProgram::useProgram() const { glUseProgram(programId_); }

std::optional<const Shader *>
GLProgram::getShader(Shader::type desiredShaderType) const {
...
std::optional<GLint>
GLProgram::getUniformLocation(std::string_view varName) const {
