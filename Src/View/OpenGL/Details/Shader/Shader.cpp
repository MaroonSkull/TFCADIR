...
Shader::Shader(const Resource &ShaderSource, Shader::type shaderType)
    : shaderType_(shaderType), source_(ShaderSource.toString()) {
  // Create shader
...
auto Shader::getShaderId() const -> decltype(shaderId_) { return shaderId_; }
...
void Shader::setUniform(std::string_view name,
                        const std::array<GLfloat, 4> &value) const {
  glUniform4f(glGetUniformLocation(shaderId_, name.data()), value.at(0),
